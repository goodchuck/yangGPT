...
import StyledAssistantChatRoom from './AssistantChatRoom.style';
...
function AssistantChatRoom({ assistantId }: Props) {
  const queryClient = useQueryClient();
  const targetAssistant: { isSuccess: boolean; results: any[] } | undefined =
  ...
  const [conversations, setConversations] = useState<
    { message: string; audioData?: any; isMe?: boolean }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (
      assistant &&
      assistant.id === 'asst_K4r7EXhRTQ0JqewkgZGHkZIV' &&
      conversations.length === 0
    ) {
      setConversations((prev) => [
        ...prev,
        { message: pdfIntroduce, isMe: false },
      ]);
    }
  }, [assistant]);
  ...
  const mutation = useMutation({
    ...
    async onSuccess(data: any) {
      ...
      for (const row of data) {
        ...
        const message = row.content[0].text.value;
  ...
  return (
    <StyledAssistantChatRoom>
      <Flex justify="center">{assistant && <h1>{assistant.name}</h1>}</Flex>
      ...
      >
        <Flex gap="middle" vertical style={{ width: '100%' }}>
          ...
          >
            <TextArea
              value={message}
              onChange={onChangeMessageEvent}
              onKeyDown={handleKeydown}
              rows={1}
              style={{ width: '97%', borderWidth: 0 }}
              autoSize={{ maxRows: 5 }}
            />
  ...
  const onClickEvent = async () => {
    if (audioRef.current) {
      if (!audioRef.current.src) {
        setDisabled(true);
        const testRes = await getTTS({ message, voice: 'nova' });
        setDisabled(false);
        ...
        >
          {!isMe && (
            <img
              width="120px"
              height="150px"
              src={isMe ? `/icons/user/ChatGPT_logo.svg` : getFace(message)}
            />
          )}
          <h3>{isMe ? 'You' : 'Assistant'}</h3>
        </Flex>
        <Flex gap="middle" style={{ width: '85%' }}>
          <TypingEffect text={message} style={{ width: '95%' }} />
...
export default AssistantChatRoom;
