...
export function FriendSection() {
  const [isOpenUserUpdateModal, setIsOpenUserUpdateModal] =
  ...
  useEffect(() => {
    if (isSuccess) {
      if (data && data.isSuccess) {
        console.log({ data });
        const filteredFriends = data.results.filter(
          (row: UserTypes) => row.user_id > 1,
        );
  ...
  return (
    <div className="friends">
      ...
      <UserUpdateModal
        open={isOpenUserUpdateModal}
        setOpen={setIsOpenUserUpdateModal}
        targetUser={targetUser}
      />
    </div>
  );
