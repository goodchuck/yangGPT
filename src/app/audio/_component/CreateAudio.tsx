...
function AudioPlayer({ buffer }: AudioPlayerProps) {
    const [audio] = useState(new Audio(URL.createObjectURL(new Blob([buffer], { type: 'audio/mp3' }))));
    ...
    return (
        <Flex gap='middle' vertical>
            ...
            <div>
                {isLoading && <Spin />}
                {!isLoading && message}
                {buffer && <AudioPlayer buffer={buffer}></AudioPlayer>}
            </div>
        </Flex>
