...
export function TextGPTForBackEnd({ user }: Props) {
  ...
  const {
    ...
    { isSuccess: boolean; results: chatRoomTypes[] },
    Object,
    { isSuccess: boolean; results: chatRoomTypes[] },
    [_1: string, _2: string]
  >({
  ...
  const mutation = useMutation({
    ...
    async onSuccess(response, variable) {
      if (response) {
        ...
        const lastUserMessage = variable.messages.at(-1);
        const lastAssMessage = { role: 'assistant', content: response };

        const result = await fetch(`/api/postgres/chatRoom/add-chat-message`, {
          method: 'post',
          headers: {
      ...
      queryKeys.forEach((queryKey) => {
        if (queryKey[0] === 'getChatRoom') {
          ...
          if (value && value.isSuccess) {
            console.log({ value });
            const shallow = JSON.parse(JSON.stringify(value));
            shallow.results[0].messages = shallow.results[0].messages.slice(
              0,
              -1,
            );
            queryClient.setQueryData(queryKey, shallow);
  ...
  async function main({
    ...
    setIsLoading(true);
    if (chatRoomData) {
      const { messages } = chatRoomData.results[0];
      ...
      {chatLoading ? (
        ...
        <>
          <Flex gap="middle" align="center">
            ...
            <h3>
              {chatRoomData &&
                chatRoomData.isSuccess &&
                chatRoomData.results.length > 0 &&
                chatRoomData.results[0].room_name}
            </h3>
          </Flex>
          {chatRoomData &&
            chatRoomData.isSuccess &&
